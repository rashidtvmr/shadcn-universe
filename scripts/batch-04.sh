#!/usr/bin/env bash
set -euo pipefail
LOG_FILE="$(pwd)/batch-04.log"
mkdir -p "$(pwd)/packages/components/batch-04"
> "$LOG_FILE"

# Process repositories 301-400
sed -n '301,400p' repos/all_github_repos.txt | while read -r URL; do
  ...
  DEST_DIR="packages/components/batch-04/$REPO_NAME"
  mkdir -p "$DEST_DIR"
...
echo "Batch 04 processing completed. Log at $LOG_FILE"