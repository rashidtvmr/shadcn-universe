#!/usr/bin/env bash
set -euo pipefail
LOG_FILE="$(pwd)/batch-02.log"
mkdir -p "$(pwd)/packages/components/batch-02"
> "$LOG_FILE"
sed -n '101,200p' repos/all_github_repos.txt | while read -r URL; do
  ...
  DEST_DIR="packages/components/batch-02/$REPO_NAME"
  mkdir -p "$DEST_DIR"
...
echo "Batch 02 processing completed. Log at $LOG_FILE"
